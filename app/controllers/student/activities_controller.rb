...
class Student::ActivitiesController < ApplicationController
  def index
    @activity = LearningObjective.find(
      params[:learning_objective_id]).activities.all
  end
  ...
  def create
    ...
    @activity = LearningObjective.find(learning_objective).activities.new(
      activity_params)

    if @activity.save
      redirect_to student_competency_learning_objective_activities_path, notice:
  ...
  def update
    ...
    if @activity.update(activity_params)
      redirect_to student_competency_learning_objective_activities_path, notice:
       'Activiteit succesvol aangepast'
    else
      render 'edit'
    end
  end
